...
public class SpiderSolitaire extends GraphicsProgram {
	...
	private boolean stackNum(int x)
	{
		...
		{
			for(int i = piles.get(x).getCards().size()-2;i>piles.get(x).getCards().size()-14;i--)
			{
	...
	public void refreshScreen()
	{
		...
		for(int i=0; i<wonAndDone.size();i++)
			add(wonAndDone.get(i),(i+2)*wonAndDone.get(i).getWidth()/2-wonAndDone.get(i).getWidth(),getHeight()*.75);
